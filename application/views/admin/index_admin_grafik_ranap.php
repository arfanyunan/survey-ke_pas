    				...
    				<div class="content-wrapper">
    					<div class="container-fluid">
							...
							<h6>Data Berdasarkan Bulan dan Tahun Tahun</h6>
									<form class="user" action="<?= base_url('admin/data_grafik_ranap'); ?>" method="POST">
										<div class="input-group shadow input-group-sm">
											<select class="form-control" id="cmb_bulan" name="bulan" aria-label="Default select example">
												<?php
													// echo '<option value="1"' . ($this->input->post('bulan') == '1' ? "selected" : (date('m') == '1' ? "selected" : "")) . '>' . "January" . '</option>';
													// echo '<option value="2"' . ($this->input->post('bulan') == '2' ? "selected" : (date('m') == '2' ? "selected" : "")) . '>' . "February" . '</option>';
													// echo '<option value="3"' . ($this->input->post('bulan') == '3' ? "selected" : (date('m') == '3' ? "selected" : "")) . '>' . "March" . '</option>';
													// echo '<option value="4"' . ($this->input->post('bulan') == '4' ? "selected" : (date('m') == '4' ? "selected" : "")) . '>' . "April" . '</option>';
													// echo '<option value="5"' . ($this->input->post('bulan') == '5' ? "selected" : (date('m') == '5' ? "selected" : "")) . '>' . "May" . '</option>';
													// echo '<option value="6"' . ($this->input->post('bulan') == '6' ? "selected" : (date('m') == '6' ? "selected" : "")) . '>' . "June" . '</option>';
													// echo '<option value="7"' . ($this->input->post('bulan') == '7' ? "selected" : (date('m') == '7' ? "selected" : "")) . '>' . "July" . '</option>';
													// echo '<option value="8"' . ($this->input->post('bulan') == '8' ? "selected" : (date('m') == '8' ? "selected" : "")) . '>' . "August" . '</option>';
													// echo '<option value="9"' . ($this->input->post('bulan') == '9' ? "selected" : (date('m') == '9' ? "selected" : "")) . '>' . "September" . '</option>';
													// echo '<option value="10"' . ($this->input->post('bulan') == '10' ? "selected" : (date('m') == '10' ? "selected" : "")) . '>' . "October" . '</option>';
													// echo '<option value="11"' . ($this->input->post('bulan') == '11' ? "selected" : (date('m') == '11' ? "selected" : "")) . '>' . "November" . '</option>';
													// echo '<option value="12"' . ($this->input->post('bulan') == '12' ? "selected" : (date('m') == '12' ? "selected" : "")) . '>' . "December" . '</option>';

													echo '<option value="1"' . ($this->input->post('bulan') == '1' ? "selected" : "") . '>' . "January" . '</option>';
...
// echo '<option value="12"' . ($this->input->post('bulan') == '12' ? "selected" : ($bulan == 01 ? "selected" : "")) . '>' . "a" . '</option>';
// echo '<option value="6"' . ($this->input->post('bulan') == '06' ? "selected" : ($bulan == 06 ? "selected" : "")) . '>' . "June" . '</option>';
														// ($this->input->post('bulan') == '12' ? "selected" : "")
												?>
											</select>
    							...
    							<div class="row mb-2">
    								<div class="col-lg-12">
										...
										<h4 class="card-title text-center">Tabel Grafik Survey Kepuasan Pasien Rawat Inap Tahun <b><?php $bulan = date('m'); $this->input->post('bulan') == TRUE ? $bulan =  $this->input->post('bulan') : $bulan = date('m'); echo $bulan; ?>-<?php $this->input->post('tahun') == TRUE ? $tahun =  $this->input->post('tahun') : $tahun = date('Y'); echo $tahun; ?></b></h4>
											...
											<table id="tabelsurvey_grafik_ranap" class="table table-bordered align-middle table-hover dt-responsive nowrap" style="width: 100%;">
    												<thead class="thead-dark">
    													<tr>
    														...
    														<th class="text-center bg_44 align-middle" rowspan="2">Bangsal</th>
    														...
    														<th class="text-center bg_44" colspan="2">Pertanyaan 6</th>
    														<th class="text-center bg_44" colspan="2">Pertanyaan 7</th>
    													</tr>
    												...
    												<tbody>
														...
														foreach ($data_admin as $dt_admin) : ?>
    														<tr>
    															...
    															<td class="text-center font-weight-bold"><?= $dt_admin['bangsal']; ?></td>
    															...
    															<td class="text-center"><?= $dt_admin['p6_ya']; ?></td>
    															<td class="text-center"><?= $dt_admin['p6_tidak']; ?></td>
    															<td class="text-center bg_4"><?= $dt_admin['p7_ya']; ?></td>
    															<td class="text-center bg_4"><?= $dt_admin['p7_tidak']; ?></td>
    														</tr>
    													<?php $no++;
														endforeach ?>
    												</tbody>
											...
											<div class="mt-4">
												<canvas id="cnv_grafik_ranap_pertanyaan"></canvas>
											</div>
    									</div>
    								</div>
    							</div>
